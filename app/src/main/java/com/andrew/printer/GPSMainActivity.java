...
public class GPSMainActivity extends AppCompatActivity {
    private static final String TAG = GPSMainActivity.class.getSimpleName();
    ...
    public void btnLabelPrint(View view) {
        Log.d("进来没===", String.valueOf(threadPool));
        threadPool = ThreadPool.getInstantiation();
        threadPool.addSerialTask(new Runnable() {
            @Override
            public void run() {
                ...
                if (DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].getCurrentPrinterCommand() == PrinterCommand.TSC) {
                    DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].sendDataImmediately(PrintContent.getLabelTest());
                } else {
                    mHandler.obtainMessage(PRINTER_COMMAND_ERROR).sendToTarget();
                }
            }
        ...
        threadPool.addSerialTask(new Runnable() {
            ...
            public void run() {
                ...
                if (DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].getCurrentPrinterCommand() == PrinterCommand.TSC) {
                    ...
                    DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].sendDataImmediately(PrintContent.printViewPhoto(bitmap));
                } else {
                    mHandler.obtainMessage(PRINTER_COMMAND_ERROR).sendToTarget();
                }
            }
        ...
        threadPool.addSerialTask(new Runnable() {
            ...
            public void run() {
                ...
                if (DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].getCurrentPrinterCommand() == PrinterCommand.CPCL) {
                    ...
                    cpcl.addCGraphics(0,0,576,PrintContent.getBitmap(GPSMainActivity.this));
                    cpcl.addPrint();
                    ...
                    // 打印图片  光栅位图  384代表打印图片像素  0代表打印模式
                    // 58mm打印机 可打印区域最大点数为 384 ，80mm 打印机 可打印区域最大点数为 576 例子为80mmd打印机
                    labelCommand.addBitmap(0,0, 576,PrintContent.getBitmap(GPSMainActivity.this));
                    ...
                    // 打印图片  光栅位图  384代表打印图片像素  0代表打印模式
                    // 58mm打印机 可打印区域最大点数为 384 ，80mm 打印机 可打印区域最大点数为 576 例子为80mmd打印机
                    esc.addRastBitImage(PrintContent.getBitmap(GPSMainActivity.this), 576, 0);
        ...
        if (resultCode == RESULT_OK) {
            switch (requestCode) {
                ...
                case Constant.USB_REQUEST_CODE: {
                    ...
                    //通过USB设备名找到USB设备
                    UsbDevice usbDevice = Utils.getUsbDeviceFromName(GPSMainActivity.this, usbName);
                    //判断USB设备是否有权限
            ...
            switch (action) {
                ...
                case DeviceConnFactoryManager.ACTION_CONN_STATE:
                    ...
                    switch (state) {
                        ...
                        case CONN_STATE_FAILED:
                            Utils.toast(GPSMainActivity.this, getString(R.string.str_conn_fail));
                            //wificonn=false;
                            tvConnState.setText(getString(R.string.str_conn_state_disconnect));
                            break;
                        default:
                ...
                case ACTION_QUERY_PRINTER_STATE:
                    if (counts >=0) {
                        if(continuityprint) {
                            printcount++;
                            Utils.toast(GPSMainActivity.this, getString(R.string.str_continuityprinter) + " " + printcount);
                        }
                        if(counts!=0) {
                            sendContinuityPrint();
                        }else {
            ...
            switch (msg.what) {
                case CONN_STATE_DISCONN://断开连接
                    ...
                    if (deviceConnFactoryManager!= null&&deviceConnFactoryManager.getConnState()) {
                        DeviceConnFactoryManager.getDeviceConnFactoryManagers()[id].closePort(id);
                        Utils.toast(GPSMainActivity.this,getString(R.string.str_disconnect_success));
                    }
                    break;
                case PRINTER_COMMAND_ERROR://打印机指令错误
                    Utils.toast(GPSMainActivity.this, getString(R.string.str_choice_printer_command));
                    break;
                case CONN_PRINTER://未连接打印机
                    Utils.toast(GPSMainActivity.this, getString(R.string.str_cann_printer));
                    break;
                ...
                case CheckWifiConnThread.PING_FAIL://WIfI断开连接
                    ...
                    Utils.toast(GPSMainActivity.this,getString(R.string.disconnect));
                ...
                case Constant.tip:
                    ...
                    Utils.toast(GPSMainActivity.this,str);
                    break;
                default:
                    new DeviceConnFactoryManager.Build()
                            //设置端口连接方式
